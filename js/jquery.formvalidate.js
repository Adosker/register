...
(function($,window,document,undefined){
	 ...
	 * tipId：提示展示区域id
	 */
	function validateFieldWithReg($element,targetValue,$tip){
		var result = true;
		for(o in CheckRules){
	 ...
	 * tipId：提示展示区域id
	 */
	function validateFieldLength($element,valiType,targetValue,targetLabel,$tip){
		var result = true,
	 ...
	 *
	 */
	function fieldInputTwiceIdentified($element,targetValue,lableText,$tip){
		 var result = true, 
	 ...
	 *	}
	 */
	function validateFieldValueUnique($element,targetValue,$tip){
		var result = true,
	...
	function validateCore($element){
		...
		//正则校验字段值 的格式
		if(!validateFieldWithReg($element,value,$tip)){
			return false;
		}

		//最小长度校验
		if(!validateFieldLength($element,'minlength',value,lableText,$tip)){
			return false;
		}

		//最大长度校验
		if(!validateFieldLength($element,'maxslength',value,lableText,$tip)){
			return false;
		}
		 
		 //2次输入值一致校验
		if(!fieldInputTwiceIdentified($element,value,lableText,$tip)) {
			return false;
		}
		 
		 //字段值唯一校验
		 if(!validateFieldValueUnique($element,value,$tip)) {
				return false;
			}
		return result;
	}
	 ...
	 */
	function validation($form,valiType){
		var result = true;
	 ...
	 * @param formId
	 */
	 $.fn.validateRealTime = function(){
	 	return validation(this,'realTime');
	 }
